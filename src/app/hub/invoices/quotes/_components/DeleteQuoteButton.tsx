...
type Props = {
  ...
  compact?: boolean;
  /** After successful delete: stay on current page (list + filters) or go to the quotes list */
  afterDelete?: "refresh" | "quotes";
};
...
export function DeleteQuoteButton({
  ...
  async function handleDelete() {
    ...
    if (afterDelete === "quotes") {
      router.push("/hub/invoices/quotes");
    }
    router.refresh();
  }
  ...
  return (
    <div className={compact ? "inline-flex flex-col items-end gap-1" : "space-y-2"}>
      ...
      >
        <span className="text-sm text-red-800">Delete {quoteNumber}?</span>
        <button
          type="button"
